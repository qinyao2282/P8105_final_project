...
```{r}
  ...
  lm(mortality_rate ~ obesity_percentage, data = final_data_income) %>% 
  broom::tidy()
...
total_lm = lm(mortality_rate ~ tobacco_consumption + median_income + obesity_percentage + sum_pm2_5 + stroke_value, data = final_data) %>%
  fortify()
